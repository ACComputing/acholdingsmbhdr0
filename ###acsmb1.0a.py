...
Informed by the SMBDIS.ASM disassembly by doppelganger.

Improvements vs original code:
- Accurate SMB1 friction/run physics (X/Shift=run, variable jump height from speed)
- Hand-crafted 1-1, 1-2, 1-3, 1-4 layouts from disassembly column data
- Power-up chain: Small -> Super (mushroom) -> Fire (flower)
- Star invincibility (flashing rainbow)
- Fireballs: Z/Ctrl when Fire Mario
- Bowser boss in every x-4 level (5 hits via fireballs/stars to defeat)
- Hammer Bros enemy type + thrown hammers
- Bowser fire breath hazard
- Flagpole score bonus based on touch height (500-5000 pts)
- 1-UP every 100 coins
- Synthesized background music themes (overworld / underground / castle / underwater / star)
- World select on title screen (press B to cycle)
- R = restart current level
- N = skip to next level
- Parallax scrolling clouds and hills (overworld)
"""
...
def build_level(w,l):
    fn=BUILDERS.get((w,l))
    return fn() if fn else build_proc(w,l)
