...
try:
    ...
    print(f"Warning: Could not generate sounds. Error: {e}")
    SND_JUMP = SND_COIN = SND_STOMP = SND_POWERUP = SND_DIE = SND_FLAGPOLE = SND_CLEAR = None
...
# -------------------------
WORLD_THEME = {
    1: 'overworld', 2: 'overworld', 3: 'overworld', 4: 'castle',
    5: 'overworld', 6: 'overworld', 7: 'overworld', 8: 'castle',
}
...
def build_level(world, level_num):
    ...
    H = 15  # rows
    W = 80 + world * 5 + level_num * 2  # cols, gets longer in later worlds
    ...
    for _ in range(num_goombas):
        for attempt in range(30):
            ...
            if any(abs(ex - p) < 3 for p in placed):
                continue
            placed.append(ex)
            enemies.append({'type': 'goomba', 'rect': pygame.Rect(ex * TILE_SIZE, (ey-1) * TILE_SIZE + 5, TILE_SIZE - 5, TILE_SIZE - 5),
    ...
    for _ in range(num_koopas):
        for attempt in range(30):
            ...
            if any(abs(ex - p) < 4 for p in placed):
                continue
            placed.append(ex)
            enemies.append({'type': 'koopa', 'rect': pygame.Rect(ex * TILE_SIZE, (ey-1) * TILE_SIZE + 5, TILE_SIZE - 5, TILE_SIZE - 5),
...
class Player:
    ...
    def collide_tiles(self, dx, dy, grid):
        ...
        # We iterate over potentially intersected tiles
        r, c = self.rect.top // TILE_SIZE, self.rect.left // TILE_SIZE
        r2, c2 = self.rect.bottom // TILE_SIZE, self.rect.right // TILE_SIZE
        
        # Ensure indices are within bounds
        min_row, max_row = max(0, r), min(H-1, r2)
        min_col, max_col = max(0, c), min(W-1, c2)
    ...
    def check_ground(self, grid):
        ...
        test = self.rect.copy()
        test.y += 2
        r, c = test.top // TILE_SIZE, test.left // TILE_SIZE
        r2, c2 = test.bottom // TILE_SIZE, test.right // TILE_SIZE
    ...
    def update(self, keys, grid, bumped_blocks):
        ...
        # Reset ground check before collision detection
        ...
        # Check if we slid off a platform
...
def update_enemy(e, grid):
    ...
    if t in ('goomba', 'koopa'):
        ...
        # Simple collision check for walls
        for col in [col_l, col_r]:
            if 0 <= col < W and 0 <= row_m < H:
                if grid[row_m][col] in SOLID_TILES:
                    if vx < 0: r.left = (col + 1) * TILE_SIZE
                    elif vx > 0: r.right = col * TILE_SIZE
                    if t == 'koopa' and e['shell']:
                        e['shell_vx'] *= -1
                    else:
                        e['vx'] *= -1
        ...
        # Edge turning for goombas / walking koopas
        if (t == 'goomba' or (t == 'koopa' and not e['shell'])) and vx != 0:
            # Check for pit
            edge_col = (r.right // TILE_SIZE) if vx > 0 else (r.left // TILE_SIZE)
            ground_check_row = (r.bottom + 5) // TILE_SIZE
            
            if 0 <= edge_col < W and 0 <= ground_check_row < H:
                if grid[ground_check_row][edge_col] not in SOLID_TILES:
                    e['vx'] *= -1
...
def player_enemy_collision(player, enemies):
    ...
    for e in enemies:
        ...
        t = e['type']
        if t == 'firebar':
            if 'rect' in e and player.rect.colliderect(e['rect']):
                if player.big:
                    player.big = False
    ...
    for row in range(H):
        for col in range(cam_col_start, cam_col_end):
            ...
            if tile == GROUND:
                ...
                pygame.draw.rect(screen, PIPE_DARK, (rx, ry, TILE_SIZE, TILE_SIZE), 3)
                if tile == PIPE_TOP_R:
                    pygame.draw.rect(screen, PIPE_GREEN, (rx, ry, TILE_SIZE + 4, TILE_SIZE // 2))
                    pygame.draw.rect(screen, PIPE_DARK, (rx, ry, TILE_SIZE + 4, TILE_SIZE // 2), 3)
...
def draw_hud(screen, world, level_num, score, coins, lives, font):
    pygame.draw.rect(screen, BLACK, (0, 0, SCREEN_WIDTH, 36))
